...
-(void) testWhenStringProperty {
    id<STMatcher> matcher = [_factory parseExpression:@"[MainClass].stringProperty == 5" error:NULL];
...
-(void) testWhenClassProperty {
    id<STMatcher> matcher = [_factory parseExpression:@"[MainClass].classProperty == 5" error:NULL];
    MainClass *mainClass = [[MainClass alloc] init];
